...
*NOTE: In the current package version only Windows functionality is implemented. On Mac, the .NET Runtime supports [using `clonefile` implicitly on .NET 8](https://github.com/dotnet/runtime/pull/79243). Issues for [Linux](https://github.com/microsoft/CopyOnWrite/issues/10) and [Mac](https://github.com/microsoft/CopyOnWrite/issues/11) are open and contributions are welcome. Consider reimplementing from the Rust package linked below.*
